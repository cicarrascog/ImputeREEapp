...
On the left side of the sidebar is possible to access the different
...
A module to upload your data. The example data, which are zircons from
[Ballard et al.
(2002)](http://link.springer.com/10.1007/s00410-002-0402-5), are
automatically loaded.

Please use the upload button in the upload panel to upload your data.
The column names of the REE **should include only the element name**
(eg. `La`, not `La_final, final_corrected` or `this_La is the good one`)
in **ppm**. **Large files will take longer to process**. Additional
columns can be included. A column with the name `rowid` (row index used
for relational data) is likely to give an error, please rename or
exclude this column.
...
less than 4 REE is not recommended.

The **R^2^** plot show the distribution of the R squared statistic for
each model (one model per zircon grain). The option button (top right
corner) allows to modify some elements on the plot (e.g. use adjusted
R^2^ instead).
...
according the R^2^ value which is only applied to the plot.
